	...
	<div class="col-12">
		<div class="card">
		    ...
		    @endif
			  ...
			  <h5>Data Klaim</h5>
			  <table class="table table-bordered">
				<tr><td style="width:250px;">{{ _lang('Deskripsi Cost Center / Cost Center') }}</td><td>{{ $spr['promo']['chanel_name'] .' / '. $spr['cost_name']['cost_numbe'] }} </td></tr>
				...
				<tr><td>{{ _lang('Nama Program') }}</td><td>{{ $spr['promo']['name'] }}</td></tr>
				<tr><td>{{ _lang('Wilayah Program') }}</td><td>{{ $spr['promo']['wilayah'] }}</td></tr>
			  ...
			  @if(!empty($spr->pay_date))
				...
				<table width="100%" class="table-bordered">
					...
					<tr>
						...
						<td><input type="text" class="form-control" name="ppn" value="{{ number_format($spr['dpp']) }}" readonly></td>
					</tr>
					<tr>
						<td>Nilai Realisasi Accounting (PPH)</td>
						<td><input type="text" class="form-control" name="pph" value="{{ number_format($spr['dpp']) }}" readonly></td>
					</tr>
					<tr>
						<td>Total Realisasi</td>
					    <td><input type="text" class="form-control" name="xswa" value="{{ number_format(($spr['dpp'] + $spr['dpp']) - $spr['dpp']) }}" readonly></td>
					</tr>
				</table>
				@endif
			  @endif
			  ...
			  <table class="table table-bordered">
				...
				@foreach($dokumen as $itemy)
					...
					@endif
				@endforeach
				<tr><td align="left"><b>Nama File Yang Dikirim</b></td><td width="70%" align="left">{{ $spr['name_document'] }}</td></tr>		
			  </table>
			  </table>
			  <br>
			  <div class="col-12">
									...
									@if($spr['internal_pend'] == 1)
										@if(Auth::user()->user_type != 'user')
											<li class="feed-item feed-item-{{ log_status($item['status']) }}">
												...
												<br>
												<span class="text">{!! ptk_status($item['status']) !!} oleh <b>{{ $item->nama2->name }}</b> "{{ $item->note_appv2 }}"</span>
											</li>
										@endif
									@else
										<li class="feed-item feed-item-{{ log_status($item['status']) }}">
									...
									@if($spr['internal_pend'] == 1)
										@if(Auth::user()->user_type != 'user')
											<li class="feed-item feed-item-{{ log_status($item['status']) }}">
												...
												<br>
												<span class="text">{!! ptk_status($item['status']) !!} oleh <b>{{ $item->nama3->name }}</b> "{{ $item->note_appv3 }}"</span>
											</li>
										@endif
									@else
										<li class="feed-item feed-item-{{ log_status($item['status']) }}">
