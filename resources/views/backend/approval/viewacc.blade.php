...
<div class="row">
	<div class="col-12">
		<div class="card">
			<div class="card-body">
			  <h4 class="card-title panel-title">{{ _lang('Finance Approval Data Klaim') }}  			  
			  ...
			  @if(Auth::user()->user_type == 'finance' || Auth::user()->user_type == 'admin' )
					@if (($spr->flag_acc == 4 && $spr->status == 'B') || ($spr->flag_acc == 4 && $spr->status == 'P'))
						<button class="btn btn-warning btn-sm float-right ajax-modal" data-title="{{ _lang('Konfirmasi Data') }}" data-href="{{ action('ApprovalController@konfirm_fat' , $id) }}">{{ _lang('Konfirmasi Data') }}</button>
					@endif
			  @endif <br>
			...
			@if (($spr->flag_acc > 4 && $spr->status == 'B') || ($spr->flag_acc > 4 && $spr->status == 'P'))
				<div class="col-12">
					<div class="card">
						...
						<div class="card-body">
							<form action="{{ route('approval.acc_approve') }}" method="post" onsubmit="return confirm('Anda Yakin ?')">
								@csrf
								<input type="hidden" name="claim_id" value="{{ $spr->id }}" >
								<table width="100%" class="table-bordered">
									<tr>
										<td>Nilai Klaim User</td>
										<td><input type="text" name="nominal" class="form-control" value="{{ number_format($spr->nominal,2) }}" readonly></td>
									</tr>
									<tr>
										<td>Nilai Realisasi Accounting (DPP)</td>
										<td><input type="text" class="form-control" name="dpp" value="{{ number_format($spr->dpp,2) }}" readonly></td>
									</tr>
									<tr>
										<td>Nilai Realisasi Accounting (PPN)</td>
										<td><input type="text" class="form-control" name="ppn" value="{{ number_format($spr->ppn,2) }}" readonly></td>
									</tr>
									<tr>
										<td>Nilai Realisasi Accounting (PPH)</td>
										<td><input type="text" class="form-control" name="pph" value="{{ number_format($spr->pph,2) }}" readonly></td>
									</tr>
									<tr>
										<td>Total Realisasi</td>
										<td><input type="text" class="form-control" name="xswa" value="{{ number_format((($spr->dpp + $spr->ppn) - $spr->pph),2) }}" readonly></td>
									</tr>
								...
								<div class="form-group">
										...
										<input type="text" class="form-control datepicker" name="pay_date" value="" >
								</div>
								<div class="form-group">
